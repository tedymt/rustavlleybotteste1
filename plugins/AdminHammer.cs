...
[Info("AdminHammer", "mvrb", "1.13.1")]
class AdminHammer : RustPlugin
{
    #region Fields
    public static AdminHammer pluginInstance;
    ...
    private readonly int layerMask = LayerMask.GetMask("Construction", "Deployed", "Default");
    ...
    private T GetConfig<T>(string name, T value) => Config[name] is null ? value : (T)Convert.ChangeType(Config[name], typeof(T));
    #endregion
    ...
    private void OnServerInitialized()
    {
        ...
        foreach (var player in BasePlayer.activePlayerList)
        {
            if (Users.Contains(player.userID))
            {
                ...
                if (player.gameObject.GetComponent<AdminHammerComponent>() is null)
                {
                    player.gameObject.AddComponent<AdminHammerComponent>();
                }
            }
        }
    ...
    private void Unload()
    {
        foreach (var ah in UnityEngine.Object.FindObjectsOfType<AdminHammerComponent>().ToList())
        {
    ...
    {
        player.gameObject.GetComponent<AdminHammerComponent>()?.Destroy();
    }
    #endregion
    ...
    private string GetAuthorized(BaseEntity entity, BasePlayer player)
    {
        ...
        foreach (ulong userId in (turret ? turret.authorizedPlayers : priv.authorizedPlayers))
        {
            authed++;
            msg += $"{authed}. {GetName(userId.ToString())}\n";
            Puts($"{authed}. {userId} {GetName(userId.ToString())}\n");
        }
    ...
    #region AdminHammer Component
    private class AdminHammerComponent : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (!player.serverInput.WasJustPressed(BUTTON.FIRE_SECONDARY) || (player.GetActiveItem() as Item)?.info.shortname != pluginInstance.toolUsed) return;

            if (currentTime - lastCheck >= 0.25f)
            {
                pluginInstance.CheckEntity(player);
